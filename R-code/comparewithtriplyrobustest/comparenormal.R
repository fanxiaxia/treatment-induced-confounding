...
triple.pre <- function(A,M,C,X,Y)
{
  ...
  etafun <- function(a)
  {
    ...
    EM <- predict(fitm,newdata = datam)
    data <- data.frame(cbind(a,C,EM,X,C))
    colnames(data) <- c("A","C","M","X","C")
...
correct.sd <- apply(result2.allcorrect,1,sd)*100
beep()
...
write.csv(correct,"comparenormal.csv")
write.csv(correct.sd,"comparenormal_se.csv")
