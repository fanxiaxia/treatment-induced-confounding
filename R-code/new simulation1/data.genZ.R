...
data_genZ <- function(n)
{
  ...
  U <- cbind(U1,U2,U3)
  pa <- expit(a0 + U %*% aU + aint12*U1*U2 + aint13*U1*U3 + aint23*U2*U3)
  A <- rbinom(n,1,prob = pa)

  
  pc <- expit(c0 + cA*A + U %*% cU + cint12 * U1 * U2 + cint13 * U1 * U3 +
                cint23 * U2 * U3 )
  pm <- expit(m0 + mA*A + U %*% mU + mint12 * U1 * U2 + mint13 * U1 * U3 +
                mint23 * U2 * U3 )
  phi <- exp(tau_0)
  ...
  for(i in 1:n)
  {
    p11.vector[i] <- p11.fun(phi,pc[i],pm[i])
  }
  ...
  EY <- y0 + yA*A + yM*M + yC*C + yU*U + yAC * A*C + yAM*A*M + yint12 * U1 * U2 + yint23 * U2 * U3 +
    yint13 * U1 * U3
  ...
  dataset <- data.frame(A,C,M,Y,U1,U2,U3)
